...
//store username/remove overlay when submit button pressed
...
//create player when page loads
function init() {
  let board = document.querySelector('.game-board');
  let myVirus = document.createElement('img');
  myVirus.setAttribute('src', '../images/redvir.png');
  myVirus.id = playerID;
  myVirus.style.position = 'absolute';
  myVirus.style.width = '100px';
  myVirus.style.height = '100px';
  myVirus.style.zIndex = '-1';
  myVirus.style.left = parseInt(Math.random() * 950)+ 'px';
  myVirus.style.top = parseInt(Math.random() * 600) + 'px';
  board.appendChild(myVirus);
}
...
function listener(evt, element, fn) {
  ...
  function move(evt) {
    ...
    var blob = document.getElementById(playerID);
    console.log(typeof playerID)
    
    switch (keycode) {
        case Key.LEFT:
          blob.style.left = parseInt(blob.style.left) - 5 + 'px';
          socket.emit('keyPress', { inputId: 'left', press : true, x: blob.style.left, y:blob.style.top });
          movePointer(keycode);
          direction = "Left";
